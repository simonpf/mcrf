...
%DIF ADD amt_manuscript_revised.tex   Sat Jan 25 13:48:22 2020
%% ---------------------------------
...
$800\ \unit{km}$. The \DIFaddbegin \DIFadd{vertical resolution of the model scenes varies between 250
and $500\ \unit{m}$ below an altitude of $18\ \unit{km}$ and decreases
steadily above that. The }\DIFaddend scenes were chosen with the aim of covering a large
...
%DIFDELCMD < %%%
\DIFdel{Observations from the ICI radiometer are simulated by performing a single,
...
$\chi^2_y$   \DIFdelbegin \DIFdel{is here used to approximate a $\chi^2$-test for the misfit between
the observations $\mathbf{y}$ and the retrieval fit $\hat{\mathbf{y}}$. Although
a formally correct }\DIFdelend \DIFaddbegin \DIFadd{corresponds to the squared error in the fitted observations
weighted by the uncertainties in each channel. It should be noted that
the quantity has no meaningful interpretation in terms of }\DIFaddend $\chi^2$\DIFdelbegin \DIFdel{-test for $\delta\mathbf{y}$ should apply a different
...
The proposed retrieval solves for \DIFdelbegin \DIFdel{distributions }\DIFdelend \DIFaddbegin \DIFadd{profiles of two degrees of freedom of the PSDs
}\DIFaddend of one frozen and one liquid hydrometeor species \DIFdelbegin \DIFdel{in the atmospheric column together }\DIFdelend \DIFaddbegin \DIFadd{along }\DIFaddend with profiles of
...
number concentration}\DIFdelend . For frozen hydrometeors,
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Since $N_0^*$ varies over several orders of magnitude it is retrieved in
...
displays }\DIFaddend the contours of \DIFdelbegin \DIFdel{the
...
species. This means that the reference IWC is the sum }\DIFdelend \DIFaddbegin \DIFadd{masses }\DIFaddend of the four frozen hydrometeor species \DIFdelbegin \DIFdel{represented }\DIFdelend in
...
in }\DIFaddend Panel (a)\DIFdelbegin \DIFdel{of the figure displays the $\chi^2_y$ value (normalized by the
dimension of the measurement space) for each profile in the scene. A high value
of $\chi^2_y$ indicates that the retrieved state is not consistent with the input observations. The $\chi^2_y$ value for }\DIFdelend \DIFaddbegin \DIFadd{, gives an indication of how well the retrievals were able to fit
the observations. For }\DIFaddend the radar-only retrieval\DIFdelbegin \DIFdel{is
...
values are normalized, the value 1 corresponds to the expected value of the approximated chi-square distribution of $\chi^2_y$. All three retrievals exhibit
a region of elevated $\chi^2_y$ values near the core of the convective system.
In particular the high values of }\DIFdelend the \DIFdelbegin \DIFdel{passive-only and combined retrievals
...
(IWP). The IWP is given in $\unit{dB}$ relative to the reference IWPsince, owing to the high dynamic range of the reference values, the curves could
otherwise not be distinguished. Although all methods reproduce }\DIFdelend \DIFaddbegin \DIFadd{around 1. This indicates that all methods fitted the observations well over
...
\DIFdelbegin \DIFdel{fairly well, the combined retrieval yields the best overall agreement with the reference values. Exceptions are the regions of high $\chi^2_y$ values where
...
reference IWCfield, the results do not reproduce }\DIFdelend \DIFaddbegin \DIFadd{with the combined retrieval consistently yielding the smallest deviations.
Larger differences between the methods are observed when comparing the retrieval
results in terms is IWC. While }\DIFaddend the vertical structure of the cloud \DIFdelbegin \DIFdel{very well. It should be noted, however, that the displayed mass-density
...
artifacts are clearly visible in
the retrieved IWC field, the retrieval
reproduces the vertical structure well. In particular, }\DIFdelend \DIFaddbegin \DIFadd{between $0^\circ N$ and $2^\circ N$. These deviations are corrected in
the results from }\DIFaddend the combined retrieval\DIFdelbegin \DIFdel{succeeds to correct some of the systematic deviations of the radar-only
retrieval : The mass density at cloud base is reduced and increased at cloud top}\DIFdelend \DIFaddbegin \DIFadd{, although certain retrieval artifacts
are visible here}\DIFaddend .
...
to the diagonal.The most distinct feature of the radar-only results, however, is the emergence of two clusters that extend along the diagonal but are displaced above respectively below it. The color coding of the markers revealsthat these clusters correspond to grid points dominated by ice for the cluster
...
%DIFDELCMD < %%%
\DIFdel{Another feature that stands out in }\DIFdelend \DIFaddbegin \DIFadd{In terms of overall accuracy, i.e. systematic deviations from the diagonal, no
clear differences between the three configurations are visible. The color-coding
reveals, however, that }\DIFaddend the radar-only \DIFdelbegin \DIFdel{results is that the retrieval does not work for graupel. This , however, can be understood by comparing the
...
%DIFDELCMD < %%%
\DIFdel{Similar to the radar-only retrieval, the results of }\DIFdelend \DIFaddbegin \DIFadd{retrieval is biased for specific
hydrometeor classes. This effect is less pronounced for }\DIFaddend the combined retrieval
...
Even though the systematic deviations observed in the }\DIFdelend \DIFaddbegin \DIFadd{and seem to be weaker also in the passive-only results. For Graupel, both the
}\DIFaddend radar-only \DIFdelbegin \DIFdel{retrieval are
...
cloud which consists of graupel and snow.
Also similar to the first scene, the
...
resolves the vertical structure of the cloud well, but overestimates the ice mass density in the scene. The combined retrieval also resolves the vertical
structure of the cloud well and corrects the overestimation observed in }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:overview} and Fig.~\ref{fig:observations_a}).
}

\DIFadd{Results of all retrievals are affected by the choice of the ice particle model,
which can cause systematic deviations especially at high IWC values. For }\DIFaddend the
radar-only results \DIFdelbegin \DIFdel{to some extent.
}%DIFDELCMD < 
...
effect is corrected by }\DIFdelend \DIFaddbegin \DIFadd{these deviations are smallest for the LargeColumnAggregate,
whereas for }\DIFaddend the combined retrieval \DIFdelbegin \DIFdel{for most of the tested particle
shapes. The exception is the GemCloudIce particle for which the retrieval of
snow particle deteriorates quite drastically}\DIFdelend \DIFaddbegin \DIFadd{the LargePlateAggregate and 8-ColumnAggregate
yield the most accurate results}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \begin{figure}[!h]
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{figure}
\DIFaddendFL \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width = 0.8\textwidth]{figures/fig10}
%DIFDELCMD < %%%
...
\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Scatter plots of the reference and retrieved ice mass densities for
  ...
  \DIFdelbeginFL \DIFdelFL{model. The following rows display the results for the
  passive-only, the radar-only and the combined retrieval}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{shapes}\DIFaddendFL .}
...
geometry and neglect multiple scattering in the radar simulations. For }\DIFdelend \DIFaddbegin \DIFadd{It should also be noted that this study does not provide }\DIFaddend a realistic assessment
...
%DIFDELCMD < %%%
\DIFdel{In addition to this, the combined retrieval also shows improved profiling capabilities
for warm and supercooled liquid clouds}\DIFdelend \DIFaddbegin \DIFadd{The sensitivity of
...
to less than $10\ \%$ for suitable choices of the particle model (Fig.~\ref{fig:boxes})}\DIFaddend .

\DIFdelbegin \DIFdel{The results presented in this study }\DIFdelend \DIFaddbegin \DIFadd{Our results }\DIFaddend particularly highlight the \DIFdelbegin \DIFdel{complementarity
...
obtained from the radar alone.A synergistic retrieval using both types of observations allows combining the high vertical resolution of the radar with the sensitivity to cloud microphysics of the passive observations, which
...
\DIFadd{The results for the second test scene obtained using the LargePlateAggregate
