...
%DIF DEL amt_manuscript.tex           Mon Mar  9 12:57:51 2020
%DIF ADD amt_manuscript_revised.tex   Mon Mar  9 16:06:32 2020
%% ---------------------------------
...
\DIFdelbegin \DIFdel{The upcoming }\DIFdelend \DIFaddbegin 
  ...
  \DIFadd{Remote sensing observations at sub-millimeter wavelength provide higher
  ...
  capacity of the observations }\DIFaddend to \DIFaddbegin \DIFadd{constrain the microphysical properties of
  hydrometeors is established. A synergistic retrieval algorithm for airborne
  observations is proposed and applied to }\DIFaddend simulated observations from a
  \DIFdelbegin \DIFdel{high-resolution atmospheric model. Comparison of the results with passive- and radar-only versions of the retrieval algorithm
...
}\DIFdelend \DIFaddbegin \DIFadd{synergistic sensor
...
outgoing radiation, constitute a }\DIFaddend part of the global hydrological cycle and \DIFdelbegin \DIFdel{due to their relation to }\DIFdelend \DIFaddbegin \DIFadd{are
coupled to }\DIFaddend the dynamics of the atmosphere \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
ice particlesis strongest for sizes roughly equal to the wavelength, $\lambda$,
...
sensors, the upcoming }\DIFdelend \DIFaddbegin \DIFadd{With the upcoming launch of the }\DIFaddend Ice Cloud Imager (ICI) \DIFdelbegin \DIFdel{will extend the microwave
frequencies available for studying clouds }\DIFdelend \DIFaddbegin \DIFadd{a new passive microwave
...
. This extension of the }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{eriksson20}}\hspace{0pt}%DIFAUXCMD
. This will narrow the size-sensitivity gap between the
infrared and traditional microwave sensors by extending the }\DIFaddend smallest currently
...
(\cite{cote98}). \DIFdelbegin \DIFdel{For this study, we restrict ourselves to two designated, two-dimensional test scenes , which are displayed in Fig.~\ref{fig:overview}. The test scenes have }\DIFdelend \DIFaddbegin \DIFadd{Two test scenes with }\DIFaddend a horizontal resolution of $1\ \unit{km}$
...
\DIFdelbegin \DIFdel{mass-size relationship are given in Tab.~\ref{tab:species_parameters}. As
...
large particles. }\DIFdelend \DIFaddbegin \DIFadd{PSD, is set
...
%DIFDELCMD < %%%
\DIFdelend Examples of particle size distributions of frozen hydrometeors are displayed in
...
concentration}\DIFaddend , whereas the \DIFdelbegin \DIFdel{function }\DIFdelend \DIFaddbegin \DIFadd{normalized }\DIFaddend shape shows less variability. Furthermore,
...
model is characterized by high particle number \DIFdelbegin \DIFdel{densities }\DIFdelend \DIFaddbegin \DIFadd{concentrations }\DIFaddend and small particle
sizes, whereas snow \DIFdelbegin \DIFdel{exhibits }\DIFdelend \DIFaddbegin \DIFadd{has }\DIFaddend lower number concentrations and larger particles.
...
\DIFaddendFL \caption{Realizations of particle size distributions from the \DIFdelbeginFL \DIFdelFL{cloud }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{test }\DIFaddendFL scenes used
  ...
  content}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{Inlets display visualizations of the particle shape assumed for each
...
\DIFdelbegin \DIFdel{A simulated observation is generated for each vertical profile in the model test scenes. The simulations apply the same microphysics scheme
...
%DIFDELCMD < %%%
\DIFdel{A number of simplifications are applied for the generation of the synthetic
cloud observations: Firstly, the }\DIFdelend \DIFaddbegin \DIFadd{An airborne sensor configuration is assumed to simulate observations. The }\DIFaddend beams
...
at typical viewing angles of imaging sensors \mbox{%DIFAUXCMD
...
}\DIFaddend higher-frequency channels \DIFdelbegin \DIFdel{. Due to }\DIFdelend \DIFaddbegin \DIFadd{and $16\ \unit{km}$ for ICI's channels. For a
spaceborne configuration, these channels were deemed unlikely to be beneficial
for a synergistic retrieval due to }\DIFaddend the very small  overlap of the
...
approximate a }\DIFdelend \DIFaddbegin \DIFadd{corresponds to
...
has no meaningful interpretation in terms of }\DIFaddend $\chi^2$\DIFdelbegin \DIFdel{-test for the misfit between
...
distribution. The $\chi^2_y$ value used here provides a less strict test in the sense that it will generally be smaller than if the formally correct covariance
matrix was used.
...
\DIFdelbegin \DIFdel{The amount of information contained in a retrieval can be quantified by
...
the DFS of the observationscan be computed as the trace of the averaging kernel
matrix
}\begin{eqnarray*}
...
%DIFDELCMD < %%%
\DIFdelend \subsubsection{Measurement space}
...
that the }\DIFdelend \DIFaddbegin \DIFadd{, which scales the particle concentration, are the two retrieved
degrees of freedom of the PSD. The other two parameters describe the }\DIFaddend shape of
...
%DIFDELCMD < %%%
\DIFdelend The temperature-dependent a priori profile for $N_0^*$ \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend frozen hydrometeors
...
restricted to the region between the freezing \DIFdelbegin \DIFdel{layer and the tropopause, whereas the retrieval of liquid }\DIFdelend \DIFaddbegin \DIFadd{level, here defined simply as the
...
%DIFDELCMD < %%%
\DIFdel{To further regularize the retrieval , }\DIFdelend \DIFaddbegin \DIFadd{level. The retrieval of
the }\DIFaddend $N_0^*$ \DIFdelbegin \DIFdel{for ice is retrieved at only 10
...
freezing layer. This was necessary to avoid }\DIFdelend \DIFaddbegin \DIFadd{parameters is further regularized by retrieving them at reduced
vertical resolution of $2\ \unit{km}$. This was found necessary to keep }\DIFaddend the
...
radar-only }\DIFaddend retrieval, also a \DIFdelbegin \DIFdel{radar-only and a passive only-retrieval are performed. The radar-only retrievaluses the same
implementation as }\DIFdelend \DIFaddbegin \DIFadd{one-moment version (M1), in only the $D_m$
...
model.
}\DIFdelend \DIFaddbegin \DIFadd{. However, higher correlations lengths are assumed, which are shown in
Tab.~\ref{tab:a_priori}
}\DIFaddend 

\DIFdelbegin \DIFdel{The setup and retrieval quantities of the passive-only retrieval are similar to the combined retrieval, with the only difference being that frozen and liquid
hydrometeors are retrieved at reduced resolution.
For ice, }\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Representation of ice particle shape}}
...
\DIFadd{The differences between hydrometeor species in the model scenario considered
...
difference between }\DIFaddend the \DIFaddbegin \DIFadd{cloudy- and clear-sky brightness temperatures ($\Delta
...
Figure~\ref{fig:contours} displays the }\DIFaddend contours of \DIFdelbegin \DIFdel{the
...
\DIFdelbegin \DIFdel{The contours of the measured active and passive cloud signals show the
}\DIFdelend \DIFaddbegin \DIFadd{Along the $\text{dBZ}_\text{max}$-contours the cloud composition changes but the
observed signal stays the same. This shows the }\DIFaddend ambiguity of the \DIFdelbegin \DIFdel{single-instrument measurements }\DIFdelend \DIFaddbegin \DIFadd{radar
...
channel provide complementary information \DIFdelbegin \DIFdel{on the parameters of }\DIFdelend \DIFaddbegin \DIFadd{two degrees of freedom of }\DIFaddend the PSD. \DIFdelbegin \DIFdel{Considering the panels corresponding to the MWI channels, the }\DIFdelend \DIFaddbegin \DIFadd{The
...
very dense clouds consisting \DIFdelbegin \DIFdel{of very }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend large particles. In contrast to that, the ICI
...
higher }\DIFdelend \DIFaddbegin \DIFadd{complementary information }\DIFaddend for less dense clouds consisting of
smaller particles.
...
and ARTS SSDB identifier }\DIFdelend \DIFaddbegin \DIFadd{listed }\DIFaddend in Tab.~\DIFdelbegin \DIFdel{\ref{tab:particles_retrieval}}\DIFdelend \DIFaddbegin \DIFadd{\ref{tab:particle_properties}}\DIFaddend . Since the results
...
content (IWC ) }\DIFdelend \DIFaddbegin \DIFadd{Results of the retrieved IWC obtained using the Large Plate Aggregate particle
mode }\DIFaddend for the first test scene \DIFdelbegin \DIFdel{is shown in Figure \ref{fig:results_a}. }\DIFdelend \DIFaddbegin \DIFadd{are displayed in Fig.~\ref{fig:results_a}. The
reference }\DIFaddend IWC is defined here as the sum of the \DIFdelbegin \DIFdel{mass densities of all frozen hydrometeor
species. This means that the reference IWC is the sum of the }\DIFdelend \DIFaddbegin \DIFadd{masses of the }\DIFaddend four frozen
hydrometeor species \DIFdelbegin \DIFdel{represented }\DIFdelend in the GEM model \DIFdelbegin \DIFdel{, whereas the retrieved IWC is
...
(a)\DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{, give an indication of how well the retrievals are able to fit the
observations. For }\DIFaddend the \DIFdelbegin \DIFdel{figure displays the
$\chi^2_y$ value (normalized by the
...
values are normalized, the value 1 corresponds to the }\DIFdelend \DIFaddbegin \DIFadd{they are around the }\DIFaddend expected value of \DIFdelbegin \DIFdel{the
...
In particular the high values of the }\DIFdelend \DIFaddbegin \DIFadd{1. This indicates that the radar-only
retrieval overfits the observations, while the }\DIFaddend passive-only and combined
...
the reference
values . Exceptions are the regions of high $\chi^2_y$ values where
...
\DIFdelbegin \DIFdel{Panel (c) shows the IWC field retrieved using the passive-only retrieval
.
Despite a certain resemblance in the overall structure between the retrieved and
reference IWCfield, the results do not reproduce the }\DIFdelend \DIFaddbegin \DIFadd{In terms of IWP, all methods provide fairly good estimates of the reference
...
results in terms of IWC. While the }\DIFaddend vertical structure of the cloud \DIFdelbegin \DIFdel{very well. It should be noted, however, that the displayed mass-density
...
%DIFDELCMD < %%%
\DIFdel{The results of }\DIFdelend \DIFaddbegin \DIFadd{between $0^\circ N$ and $2^\circ N$. These deviations are corrected in the
results from }\DIFaddend the combined retrieval\DIFdelbegin \DIFdel{are displayed in panel (e). Although some
artifacts are
clearly visiblein the retrieved IWC field, the retrieval
...
}\DIFdelend \DIFaddbegin \DIFadd{, although certain retrieval artifacts are
visible.
}\DIFaddend 
...
\DIFaddendFL \caption{Results of the ice hydrometeor retrieval for the first test scene \DIFaddbeginFL \DIFaddFL{using
  the Large Plage Aggregate particle shape}\DIFaddendFL . Panel (a) displays the value of the
...
}\DIFaddend radar-only \DIFdelbegin \DIFdel{retrieval are more accurate than the
...
is the emergence of two clusters that extend along the diagonal but are displaced above respectively below it.
The color coding of the markers reveals that these clusters correspond to grid points dominated by ice for the
cluster
below the diagonal and snow for the cluster located above the diagonal. This
indicates that the radar-only }\DIFdelend retrieval \DIFdelbegin \DIFdel{systematically underestimates mass
densities for cloud ice but overestimates the mass density of snow. The effect
is observed for all tested particle shapes and thus likely independent of it. In general, the radar-only results exhibit only very weak dependency on the particle model, making the resultsfor different particle shapes virtually
indistinguishable.
...
\DIFdelbegin \DIFdel{Another feature that stands out in the }\DIFdelend \DIFaddbegin \DIFadd{Comparing the results for different particle models, a clear dependency is
evident in the passive-only and the combined results while the }\DIFaddend radar-only
\DIFdelbegin \DIFdel{results is that the retrieval
...
Even though the systematic deviations observed in the radar-only }\DIFdelend retrieval \DIFdelbegin \DIFdel{are
...
particle model are shown in Fig. ~\ref{fig:results_b}. As mentioned above, the results are qualitatively very similar to those of the first scene. Also here,
the final OEM cost, shown in Panel~(a), displays a region of increased cost for
the }\DIFdelend \DIFaddbegin \DIFadd{is affected the least. For the combined and }\DIFaddend passive-only
\DIFdelbegin \DIFdel{and combined retrievals. This is again a region of very dense
...
structure of the cloud well and corrects the overestimation observed in the radar-only resultsto some extent}\DIFdelend \DIFaddbegin \DIFadd{retrieval, the effect is consistent across the methods, with the GEM Cloud Ice
...
\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Results of the ice hydrometeor retrieval }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Retrieved IWC plotted against reference IWC }\DIFaddendFL for the \DIFdelbeginFL \DIFdelFL{second test scene.
  ...
  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Each row }\DIFaddendFL shows \DIFdelbeginFL \DIFdelFL{retrieved IWP }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the retrieval results for the particle shape
  shown }\DIFaddendFL in \DIFdelbeginFL \DIFdelFL{dB relative to }\DIFdelendFL the \DIFdelbeginFL \DIFdelFL{reference IWP}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{first panel}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{Panel (c) displays
  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The following panels show }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{reference mass concentrations from }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{retrieval results for
  }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{model scene. Panel }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{passive-only }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{d}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{first column}\DIFaddendFL ), \DIFaddbeginFL \DIFaddFL{the radar-only }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{e}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{second column}\DIFaddendFL ) and \DIFaddbeginFL \DIFaddFL{the
  combined retrieval }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{f}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{third column}\DIFaddendFL )\DIFdelbeginFL \DIFdelFL{display }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{. Markers are colored according to }\DIFaddendFL the
  \DIFdelbeginFL \DIFdelFL{retrieval results for }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{prevailing hydrometeor type at }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{passive-only}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{corresponding grid point in the test
  scene. Due to their sparsity}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{radar-only and combined
  retrieval, respectively}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{markers corresponding to graupel are drawn at
...
\DIFdelbegin \DIFdel{Scatter plots for the retrieval results from the second scene are shown in
...
%DIFDELCMD < %%%
\DIFdelend To summarize retrieval performance for all tested retrieval methods and particle
...
\DIFdelbegin \DIFdel{spread of the retrieval errors of the }\DIFdelend \DIFaddbegin \DIFadd{two-moment }\DIFaddend radar-only \DIFaddbegin \DIFadd{retrieval performs similar to the combined retrieval in
...
consist mostly of snow, the bias of the radar-only }\DIFaddend retrieval \DIFdelbegin \DIFdel{is lower in the second scene, the combined retrieval
yields smaller systematic
errors }\DIFdelend \DIFaddbegin \DIFadd{with respect to
...
\DIFaddendFL \caption{Scatter plots of the retrieved particle number \DIFdelbeginFL \DIFdelFL{densities }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{concentrations }\DIFaddendFL at grid points
...
of ice particles. The information }\DIFdelend \DIFaddbegin \DIFadd{To quantify the information }\DIFaddend content of the \DIFdelbegin \DIFdel{retrievals can be assessed more
...
Fig.~\DIFdelbegin \DIFdel{\ref{fig:dofs}.
...
with the DFS values of }\DIFdelend \DIFaddbegin \DIFadd{For the radar-only retrieval the information content is significantly
higher, on the order of 20 degrees of freedom, but the major part of it is
attributed to }\DIFaddend the \DIFdelbegin \DIFdel{active- and passive-only retrieval shows that the
observations contain a certain degree of redundancy leading to a lower combined
DFS value than the direct sum of the two. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The grouping into retrievalquantities furthermore reveals that the largest
increase in the information content comes from water vapor, which is not
retrieved in }\DIFdelend \DIFaddbegin \DIFadd{$D_m$ parameter. For the combined retrieval, the total
...
zero.
For }\DIFdelend \DIFaddbegin \DIFadd{and combined retrievals.
For LCWC, }\DIFaddend the \DIFdelbegin \DIFdel{$D_m$ parameter, a small decrease is observed with respect to the radar-only retrieval for both scenes. Since the calculation of the AVK involves
the forward model Jacobian, this effect must be related to the non-linearity of the forward model}\DIFdelend \DIFaddbegin \DIFadd{information content of the combined observations is increased
...
channels }\DIFaddend are displayed in \DIFdelbegin \DIFdel{Panel~(a) of }\DIFdelend Fig.~\DIFdelbegin \DIFdel{\ref{fig:costs}. As can be seen from this plot,
...
explains the bad performance observedin }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:misfit}. Each test scene contains a
...
directly to }\DIFaddend the \DIFdelbegin \DIFdel{results presented above. Similarly,
the bad performance of the GemSnow model in terms of retrieved IWC and IWP is likely due to it not covering small particle sizes. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Also displayed in Fig.~\ref{fig:costs} (panel (b) and (c)) are }\DIFdelend \DIFaddbegin \DIFadd{assumed particle shape. In contrast to this, the retrieval fits
the observations well in the remaining parts of the scene. The exception is }\DIFaddend the
\DIFdelbegin \DIFdel{$\chi^2_y$
...
the results. In the first test scene, the best fit is obtained by the
GemSnow,
...
the second scene, similar results are observed . Here, the
GemSnow particle
consistently gives the lowest $\chi^2_y$ value but comparison with
Fig.
~\ref{fig:boxes} clearly shows that it does not yield the best retrieval
performance.
}\DIFdelend \DIFaddbegin \DIFadd{GEM Graupel particle, for which quite significant misfits are observed in the
...
\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Mass-size relations }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Residuals of the fitted observations. First row of panels shows the
  profile root mean squared error }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{Panel }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{RMS) between fitted }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{a}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\hat{\mathbf{y}}$}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{) }\DIFdelendFL and
  \DIFdelbeginFL \DIFdelFL{$\chi^2_y$ values }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{true ($\mathbf{y}$) radar observations }\DIFaddendFL for the two test scenes\DIFdelbeginFL \DIFdelFL{(Panel (b) and (c))}\DIFdelendFL . \DIFdelbeginFL \DIFdelFL{The final cost curves where smoothed using }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Rows 2, 3 and
  4 show the residual $\Delta \mathbf{y} = \hat{\mathbf{y}} - \mathbf{y}$ for }\DIFaddendFL a
  \DIFdelbeginFL \DIFdelFL{running average filter }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{selection }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{a width of 20 profiles}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{ICI channels}\DIFaddendFL .}
...
applications of the
combined retrieval approach. Panel~(a) of the
figure shows
the reference and retrieved column-integrated LWC, here referred to as liquid }\DIFdelend \DIFaddbegin \DIFadd{. For the
...
retrieved liquid cloud }\DIFaddend water path (\DIFdelbegin \DIFdel{LWP) . Although the total LWC is still underestimated, the combined
observations clearly improve the LWP retrieval in all regions except those covered
by thick clouds.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Panel (b) displays the reference LWC drawn as contours on top of the total
...
retrieval to better locate cloud water in }\DIFdelend \DIFaddbegin \DIFadd{LCWP) in }\DIFaddend the \DIFdelbegin \DIFdel{atmospheric column}\DIFdelend \DIFaddbegin \DIFadd{northern part of the scene. It
...
the beams. \DIFdelbegin \DIFdel{On the other hand, a }\DIFdelend \DIFaddbegin \DIFadd{A }\DIFaddend source of forward model error was included by applying a more
complex microphysics scheme in the simulations than the one used in the
retrieval. This \DIFdelbegin \DIFdel{allows assessing }\DIFdelend \DIFaddbegin \DIFadd{permits assessment of }\DIFaddend the retrieval error caused by the
...
\ref{fig:results_scatter_a_2} and
\ref{fig:results_scatter_b_1} revealed }\DIFdelend \DIFaddbegin \DIFadd{shown in Fig.~\ref{fig:results_scatter_a} and
...
test scene. This instead indicates that the a priori assumptions used }\DIFaddend in the
retrieval do not \DIFdelbegin \DIFdel{fit the specific microphysical
...
throughout the whole scene. Viewed }\DIFdelend \DIFaddbegin \DIFadd{provide a sufficiently good description of how the $D_m$ and
$N_0^*$ parameters of the PSD co-vary and that the radar-only observations alone
do not constrain both of them well enough. This is plausible also }\DIFaddend from an
information content perspective \DIFdelbegin \DIFdel{, this
is plausible }\DIFdelend since the radar provides only one piece of
independent information at each range gate, which is insufficient to determine
the two degrees of freedom ($N_0^*$ and $D_m$) of the PSD. \DIFdelbegin \DIFdel{The information on the second degree of
...
used in the DARDAR retrievals.Also here it should be
...
development of a combined cloud retrieval.The fact that the
a priori
assumptions used in the DARDAR retrieval do not agree with the microphysical
properties of }\DIFdelend \DIFaddbegin \DIFadd{This hypothesis is
...
\ref{fig:results_a}), the \DIFdelbegin \DIFdel{analysis of the
results of the combined retrieval presented in Figs.~\ref{fig:results_scatter_a_1} , \ref{fig:results_scatter_a_2}
}\DIFdelend \DIFaddbegin \DIFadd{best overall performance for IWC and IWP is from the
combined retrieval as shown in Fig.~\ref{fig:results_scatter_a} }\DIFaddend and in
particular \DIFdelbegin \DIFdel{\ref{fig:boxes} shows that it yields, at least for most of the
tested particle models, the best retrieval performance for IWC and IWP}\DIFdelend \DIFaddbegin \DIFadd{Fig.~\ref{fig:boxes}}\DIFaddend . The benefit of the combined observations is
even more pronounced in the retrieved number \DIFdelbegin \DIFdel{density fields }\DIFdelend \DIFaddbegin \DIFadd{concentrations
}\DIFaddend (Fig.~\ref{fig:results_nd_a}). Here, the passive- and radar-only \DIFdelbegin \DIFdel{retrieval }\DIFdelend \DIFaddbegin \DIFadd{retrievals
}\DIFaddend showed little to no skill in retrieving the \DIFdelbegin \DIFdel{particle }\DIFdelend number concentrations. \DIFdelbegin \DIFdel{The combined retrieval , however, }\DIFdelend \DIFaddbegin \DIFadd{In contrast
to this, the combined retrieval }\DIFaddend was able to reproduce the general structure of
the number concentration \DIFdelbegin \DIFdel{fields }\DIFdelend \DIFaddbegin \DIFadd{field }\DIFaddend in regions where the cloud composition is
homogeneous (Fig.~\ref{fig:results_nd_scatter_a}). In particular this \DIFdelbegin \DIFdel{showed }\DIFdelend \DIFaddbegin \DIFadd{shows }\DIFaddend that
the combined retrieval is able to distinguish the microphysical properties of
ice and snow in the \DIFdelbegin \DIFdel{model. }%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{Although the }\DIFdelend \DIFaddbegin \DIFadd{test scenes. Instead of relying on the a priori, the
}\DIFaddend combined retrieval can \DIFdelbegin \DIFdel{reduce systematic errors in the retrieved
...
\DIFadd{The a priori assumptions used in this study are similar to those of the
...
unsuitable particle shape is chosen, }\DIFaddend the \DIFdelbegin \DIFdel{assumed particle shape}\DIFdelend \DIFaddbegin \DIFadd{induced errors may actually outweigh
...
particle properties displayed in Fig.~\ref{fig:particle_properties}}\DIFaddend , \DIFdelbegin \DIFdel{it would be desirable to know which of the properties of a particle model are most critical for its representativeness. Five
...
GemCloudIce model, }\DIFdelend a likely
...
%DIFDELCMD < %%%
\DIFdel{Furthermore, it has been briefly investigated whether the
goodness of the fit to
...
and how information on the ice particle shapecan be extracted from microwave
observationsof ice particles}\DIFdelend \DIFaddbegin \DIFadd{assumed particle shape and indicates that
...
thus requiring it to be determined a priori}\DIFaddend .
...
cloud is required to yield the same passive cloud signal. This }\DIFdelend \DIFaddbegin \DIFadd{passive-only retrieval shows
sensitivity to LCWC, the results are less robust than those of the combined
retrieval. This }\DIFaddend shows that combined \DIFdelbegin \DIFdel{radar and microwave radiometer observationscan also }\DIFdelend \DIFaddbegin \DIFadd{millimeter and sub-millimeter radiometers,
...
\DIFdelbegin \DIFdel{Although no satisfactory results were obtained from the water vapor retrieval, }\DIFdelend \DIFaddbegin \DIFadd{For the water vapor retrieval, no significant improvements in }\DIFaddend the \DIFdelbegin \DIFdel{retrieval results still indicate sensitivity of this setup for retrieving
...
radar-only retrievaldid converge well in most regions where the final cost of
the combined
retrieval remained high. The inability of the retrieval to fit the
observations indicates additional information that is contained in }\DIFdelend \DIFaddbegin \DIFadd{combined
...
for the climate of the arctic. The sensitivity to LCWC }\DIFaddend of the passive
observations \DIFdelbegin \DIFdel{, which
yields more accurate retrievalsof IWC, IWP and particle number densities}\DIFdelend \DIFaddbegin \DIFadd{is also a promising indication for combined ICI/MWI retrievals}\DIFaddend .
...
the infrared and visible domain}\DIFdelend \DIFaddbegin \DIFadd{Ultimately, spaceborne combined radar and sub-millimeter observations can be a
...
Earth's NexT-generation ICE missiion (ENTICE) described in \mbox{%DIFAUXCMD
\citet{jiang19}}\hspace{0pt}%DIFAUXCMD
. The
...
because of its ability to better determine the microphysical properties of hydrometeors even inside of thick clouds. Since such information is currently
...
%DIFDELCMD < %%%
\DIFdelend %% The following commands are for the statements about the availability of data sets and/or software code corresponding to the manuscript.
...
\DIFaddbegin \section{\DIFadd{Results from second test scene}}
\label{app:results_b}
...
\DIFaddend %% Please add \clearpage between each table and/or figure. Further guidelines on figures and tables can be found below.
