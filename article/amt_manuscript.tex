...
\begin{abstract}
  With the upcoming launch of the Ice cloud imager (ICI) on board the second
  generation of European operational meteorological satellites (MetOpSG),
  space-borne sub-millimeter imaging of clouds will gain operational prominence.
  To investigate what can be learned from this newly available frequencies about
  ice clouds, this study explores the potential of combining observations from a
  sub-millimeter wave radiometer with those from a $94 \unit{GHz}$ cloud-radar
  ...
  radar and the passive only retrievals can help to better constrain the
  particle size distribution of the ice cloud. The feasibility of the combined
  retrieval is demonstrated by applying a one-dimensional, variational
  ...
  radar-only versions of the retrieval algorithm clearly indicate that the
  synergistic information in the active and passive observations helps to better
  constrain the microphysical properties of frozen hydrometeors. In addition to
  that, the synergistic retrieval shows potential to distinguish and retrieve
  liquid cloud water. The results of this study clearly demonstrate the
  potential of combined active and passive millimeter and sub-millimeter
  observations. As observational concept, the joint retrievals thus
  constitute a potential way forward towards reducing the uncertainties in the
  observational record of ice in the atmosphere.
\end{abstract}
...
does not work for graupel. This, however, can be understood by comparing the radar
reflectivities shown in Fig.~\ref{fig:observations_a} with the cloud structure
displayed in Fig.~\ref{fig:overview}. It becomes apparent that graupel in this
scene is located where the radar signal is fully attenuated. Hence there is no
signal to retrieve the mass density from, explaining the bad performance for these
grid points.
...
in closer towards the diagonal. This clearly indicates that the complementary
information content in the passive observations helps to better constrain the
cloud microphysics leading to better retrieval performance.

Nonetheless, the results for the GemCloudIce particle stand out in the results.
Although for most particle shapes, the systematic deviations observed in the
radar-only retrieval seem to be reduced, for this specific shape they are instead
increased. This is caused  mostly by the effect the particle shape has the
