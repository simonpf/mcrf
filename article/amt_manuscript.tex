...
\begin{abstract}
  ...
  the atmosphere. To investigate what can be learned about ice clouds from these newly
  available frequencies, this study explores the potential of combining
  observations from a sub-millimeter wave radiometer with those from a $94
  \ \unit{GHz}$ cloud-radar to retrieve frozen hydrometeors. Starting from a
  simplified numerical experiment, it is shown that the complementary
  information content in the radar and radiometer observations can help to
  better constrain the particle size distribution of the ice cloud. The
  feasibility of the combined retrieval is demonstrated by applying a
  ...
  hydrometeors. In addition to that, the synergistic retrieval shows potential
  to distinguish and retrieve liquid clouds. The results of this study clearly
  demonstrate the potential of combining radar and sub-millimeter radiometer
  observations for the study of ice clouds. As observational concept, the
  combined retrievals point out a potential way forward towards reducing
  uncertainties in the observational record of ice in the atmosphere.
\end{abstract}
...
influence the Earth's energy budget. Furthermore, as part of the global
hydrological cycle and due to their relation to the dynamics of the atmosphere,
observations of ice clouds provide important information to constrain the state
of the atmosphere in numerical weather prediction models \citep{geer} as
well as to validate predictions from climate models \citep{waliser09}.
...
\citep{eliasson11,duncan18a}. What makes the remote sensing of ice from space so
difficult is the large variability in sizes and concentrations in which ice
particles occur in the atmosphere. The spectrum of ice crystal sizes, that
...
counterparts, ultimately they are  limited by the same principles.
...
In addition to the ICI radiometer, the experiments presented here make use
of the upper channels of the Microwave Imager (MWI) radiometer. MWI will be
flown on the same satellite as ICI so that co-located observations from the two
sensors will be readily available. The passive observations are combined with a W-band
cloud radar similar to the CloudSat Cloud Profiling Radar (CPR)
...
base (ARTS SSDB, \citet{eriksson18}).
...
mean diameter $D_m$ and the normalized number density $N_0^*$. At each height level
and hydrometeor species, only these two scaling parameters are retrieved while he
shape of the PSD, described by the parameters $\alpha$ and $\beta$, remains
fixed. The principle is illustrated in Fig.~\ref{fig:psds_retrieval}. The plot
...
that the shape of the PSD becomes independent of the retrieved mass density
and number concentration. For frozen hydrometeors, the values of the shape parameters
...
between the freezing layer and $230\ \unit{K}$-isotherm. In contrast to the
...
vertical scaling factors of the PSD of the cloud. To investigate this, the
$N_0^*$ and $D_m$ parameters of the homogeneous cloud layer are varied and
observations of the cloud layer are simulated. Figure \ref{fig:contours} displays
the simulated cloud signal, i.e. the brightness temperature difference between
clear sky and cloudy sky simulation, as filled contours for a selection of
channels of the MWI and ICI sensors. For given values of $N_0^*$ and $D_m$, the
ice mass density is given by the relation
\begin{align}
...
each other. The panels in Fig.~\ref{fig:contours} thus give an indication to
...
are shown for the first test scene Fig.~\ref{fig:observations_a}. Independent
Gaussian noise with standard deviations according to sensor specifications has
been added to the simulated observations to account for sensor noise. To model
