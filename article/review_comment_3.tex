...
As mentioned above we will revise the particle habits used in the retrieval, but
we expect that particle shape will continue to have a smaller impact on our
radar-only retrieval. On the other hand, we see significant errors that can be
attributed to the particle size distribution (PSD). However, it is difficult in
...
\todo{Not sure if necessary to mention $D_\text{max}$, since this might only
  confusion.}
